...
, debug_draw(NULL), dispatcher(NULL), pairCache(NULL), constraintSolver(NULL), collisionConfiguration(NULL)
{
	debug_draw = NULL;
	debug = true;
...
update_status ModulePhysics3D::Update(float dt)
{
	...
	if(debug == true)
	{
		...
		if(App->input->GetKey(SDL_SCANCODE_1) == KEY_DOWN)
		{
			...
			matrix.look(-App->camera->Z, App->camera->Y, App->camera->X);
			...
			btSphereShape* shape = new btSphereShape(2.0f);
